...
class ProcessadorDePendenciasGUI < FXMainWindow
  ...
  def initialize(app)
	...
	controlGroup = FXGroupBox.new(controls, "Selecione o banco sendo processado",
      GROUPBOX_TITLE_CENTER|FRAME_RIDGE|LAYOUT_SIDE_TOP|LAYOUT_FILL_X)
	optionsPopup = FXPopup.new(controlGroup)
	["Detecção automática", "BMG", "Bradesco", "Santander"].each do |opt|
		FXOption.new(optionsPopup, opt)
	end
	@bank_select = FXOptionMenu.new controlGroup, optionsPopup, opts: FRAME_RAISED|FRAME_THICK|ICON_BEFORE_TEXT|LAYOUT_FILL_X
	
	controlGroup = FXGroupBox.new(controls, "Selecione a planilha a ser processada",
      GROUPBOX_TITLE_CENTER|FRAME_RIDGE|LAYOUT_SIDE_TOP|LAYOUT_FILL_X)
	...
	footer = FXVerticalFrame.new(self,
      LAYOUT_SIDE_BOTTOM|LAYOUT_FILL_X,
	  padLeft: 40, padRight: 40, padTop: 10, padBottom: 10)
	 
	FXButton.new(footer, "Copiar para área de transferência",
	  opts: LAYOUT_SIDE_BOTTOM|FRAME_RAISED|FRAME_THICK|LAYOUT_FILL_X)
